...
- 목표 KPI/합격 기준: 기존 회귀 및 음력 변환 평가셋 실패 0건, 실제 OpenAI 대화 13/13 통과.
- 검증 결과: Python 50/50, 프론트엔드 12/12, TypeScript/Vite 빌드 성공. 실제 OpenAI 평가셋은 특정 일정 카드와 음력 날짜 추가 답변을 포함한 13건이다.
...
- 괄호 표기: `(음력)`, `（음력）`, `(음력 10/2)`, `(음력) 10/2`, 내부 공백을 인식한다. 특정 일정 변환은 그 일정만 처리하며 카드의 날짜를 새 조회 기간으로 사용하지 않는다. 월·일이 없으면 해당 일정만 질문하고 `음력 10월 2일 평달이야` 같은 추가 답변을 같은 대화에서 표시 변환에 적용한다.
- 한계: 조회된 원본 일정만 변환하므로 원래 조회 범위 밖에 등록된 음력 일정까지 발견한다고 보장하지 않는다. `(음력)`만으로 등록일이 음력인지 이미 변환된 양력인지 추측하지 않는다. 추가 정보는 브라우저 대화에만 유지하며 원본 일정에는 쓰지 않는다. 실제 사용자 Google 계정 E2E는 미검증이다.
