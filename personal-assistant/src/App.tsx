...
import { answerQuestion, executeCreate, interpret } from './services/assistantService'
import type { Answer, CalendarItem, CalendarSnapshot, ConversationContext, CreateRequest, MailItem } from './types'
...
export default function App() {
  ...
  async function connect(kind: 'calendar' | 'gmail') {
    try { setNotice(''); if (kind === 'calendar') { await auth.connectCalendar(); setCalendarSnapshot(null); setEvents([]); setContext(previous => ({ importantCategories: previous.importantCategories })) } else await auth.connectGmail(); setNotice(`${kind === 'calendar' ? 'Calendar' : 'Gmail'} 연결이 완료됐어요.`) }
  ...
  async function run(commandText = input) {
    ...
    try {
      const teamMembers = teamInput.split(/[,;\s]+/).filter(Boolean)
      const command = await interpret(question, { ...context, teamMembers })
      if (command.action === 'calendar_read') {
        ...
        const result = command.reusePrevious && calendarSnapshot ? calendarSnapshot.results : await listEvents(getToken(SCOPES.calendarRead), command.range)
        // Preserve all source results, including events hidden by the current filter.
        setCalendarSnapshot({ command, results: result })
        setContext(previous => ({ ...previous, calendarCommand: command, importantCategories: command.importantCategories, lastTopic: 'calendar', pendingRequest: undefined }))
        const response = await answerQuestion(question, command, result)
        setEvents(response.events || [])
        addAnswer(question, response.answer)
      } else if (command.action === 'preference_update') {
        setContext(previous => ({ ...previous, importantCategories: command.importantCategories }))
        addAnswer(question, command.clarification || '중요 일정 기준을 이 대화에 반영했어요.', 'info')
        ...
        setPending({ ...command.create, operationId: crypto.randomUUID() })
        setContext(previous => ({ ...previous, pendingRequest: undefined }))
        addAnswer(question, `“${command.create.title}” 일정을 ${formatDate(command.create.start)}부터 ${formatDate(command.create.end)}까지 만들까요?${command.create.attendees?.length ? ` 팀 초대: ${command.create.attendees.join(', ')}.` : ''} 아래에서 확인해 주세요.`, 'info')
        ...
        setMails(items)
        setContext(previous => ({ ...previous, lastTopic: 'mail', pendingRequest: undefined }))
        const response = await answerQuestion(question, command, { mails: command.action === 'reply_draft' ? items.slice(0, 1) : items })
        addAnswer(question, response.answer)
      } else {
        setContext(previous => ({ ...previous, pendingRequest: command.pendingRequest }))
        addAnswer(question, command.clarification || '요청한 작업과 날짜를 좀 더 구체적으로 알려 주세요.', 'info')
      }
  ...
  async function confirmCreate() {
    ...
    try {
      ...
      const response = await executeCreate(requested)
      setEvents(previous => [response.event, ...previous.filter(event => event.id !== response.event.id)])
      setCalendarSnapshot(null)
      setContext(previous => ({ importantCategories: previous.importantCategories }))
      addAnswer('일정 생성 및 팀 공유 확인', response.answer)
    } catch (error) { addAnswer('일정 생성 확인', messageOf(error), 'error'); setPending(requested) }
    finally { setBusy(false) }
  }
  function signOut() { auth.signOut(); setEvents([]); setMails([]); setPending(null); setCalendarSnapshot(null); setContext({}); setTeamInput(''); setNotice('Google 연결을 해제했어요.') }
  ...
  return <div className="app-shell">
    ...
    <main className="layout">
      ...
      <section className="connect-panel" aria-label="팀 초대 설정"><div><h2>공유할 팀</h2><p>회의 공유 요청 시 캘린더 초대를 보냅니다. 이메일은 이 화면을 닫거나 새로고침하면 사라집니다.</p><label htmlFor="team-emails">팀 이메일 (쉼표로 구분)</label><input id="team-emails" value={teamInput} onChange={event => setTeamInput(event.target.value)} placeholder="member1@example.com, member2@example.com" disabled={busy} /></div></section>
      ...
      {pending && <section className="confirm-card" role="dialog" aria-label="일정 생성 확인"><div><strong>{pending.attendees?.length ? '일정을 만들고 팀에 초대 알림을 보낼까요?' : '일정을 추가할까요?'}</strong><p>{pending.title} · {formatDate(pending.start)} – {formatDate(pending.end)}</p>{!!pending.attendees?.length && <p>초대 대상: {pending.attendees.join(', ')}</p>}</div><div><button onClick={() => setPending(null)}>취소</button><button className="primary" onClick={() => void confirmCreate()} disabled={busy}>{pending.attendees?.length ? '확인하고 생성·공유' : '확인하고 추가'}</button></div></section>}
      ...
      <footer>질문·조회 범위·중요도 기준·팀 이메일과 메일 미리보기는 OpenAI에 전달됩니다. 일정 목록은 서버에서 원본으로 정리합니다. 연결 권한과 대화 정보는 새로고침하면 사라집니다.</footer>
    </main>
  </div>
}
