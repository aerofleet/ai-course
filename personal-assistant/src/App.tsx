...
export default function App() {
  ...
  async function run(commandText = input) {
    ...
    try {
      const command = await interpret(question, context)
      if (command.action === 'calendar_read') {
        ...
        setContext(previous => ({ ...previous, calendarCommand: command, importantCategories: command.importantCategories, lastTopic: 'calendar' }))
        ...
        setMails(items)
        setContext(previous => ({ ...previous, lastTopic: 'mail' }))
  ...
  function signOut() { auth.signOut(); setEvents([]); setMails([]); setPending(null); setCalendarSnapshot(null); setContext({}); setNotice('Google 연결을 해제했어요.') }
  ...
  return <div className="app-shell">
    ...
    <main className="layout">
      ...
      <footer>질문·조회 범위·중요도 기준과 메일 미리보기는 OpenAI에 전달됩니다. 일정 목록은 서버에서 원본으로 정리합니다. 연결 권한과 대화 정보는 새로고침하면 사라집니다.</footer>
    </main>
  </div>
}
