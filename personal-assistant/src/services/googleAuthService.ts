...
const gmailClientId = import.meta.env.VITE_GOOGLE_GMAIL_CLIENT_ID as string | undefined
...
export async function requestScope(scope: string): Promise<void> {
  const selectedClientId = scope === SCOPES.gmailRead ? (gmailClientId || clientId) : clientId
  if (!selectedClientId || !selectedClientId.endsWith('.apps.googleusercontent.com')) throw new Error('Google 웹 OAuth Client ID가 올바르게 설정되지 않았어요. README의 설정 방법을 확인해 주세요.')
  const google = (window as GoogleWindow).google
  if (!google?.accounts.oauth2) throw new Error('Google 로그인 도구를 불러오지 못했어요. 인터넷 연결을 확인하고 새로고침해 주세요.')
  await new Promise<void>((resolve, reject) => {
    const client = google.accounts.oauth2.initTokenClient({ client_id: selectedClientId, scope, callback: response => {
